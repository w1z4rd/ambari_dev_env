...
sudo apt-get remove -y krb5-kdc krb5-config krb5-user krb5-clients krb5-rsh-server krb5-admin-server --purge

# Install packages
echo "Installing Kerberos Packages"
sudo apt-get install -y krb5-kdc krb5-config krb5-admin-server
...
sudo cat >/etc/krb5kdc/kadm5.acl <<EOF
*/admin@EXAMPLE.COM	*
EOF
